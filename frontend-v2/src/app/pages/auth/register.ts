                        ...
                        <p-fluid>
                            <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
                                <div class="flex flex-col gap-2">
                                    <label for="nome" class="text-surface-900 dark:text-surface-0 font-medium">Nome</label>
                                    <input pInputText id="nome" type="text" placeholder="Digite seu nome" formControlName="nome" />
                                    @if (registerForm.get('nome')?.invalid && registerForm.get('nome')?.touched) {
                                        <small class="text-red-500">Nome é obrigatório (mínimo 2 caracteres)</small>
                                    }
                                </div>
                                ...
                                <div class="flex flex-col gap-2">
                                    <label for="senha" class="text-surface-900 dark:text-surface-0 font-medium">Senha</label>
                                    <p-password id="senha" formControlName="senha" placeholder="Digite sua senha" [toggleMask]="true" [feedback]="true" />
                                    @if (registerForm.get('senha')?.invalid && registerForm.get('senha')?.touched) {
                                        <small class="text-red-500">Senha é obrigatória (mínimo 6 caracteres)</small>
                                    }
                                </div>

                                <div class="flex flex-col gap-2">
                                    <label for="confirmarSenha" class="text-surface-900 dark:text-surface-0 font-medium">Confirmar Senha</label>
                                    <p-password id="confirmarSenha" formControlName="confirmarSenha" placeholder="Confirme sua senha" [toggleMask]="true" [feedback]="false" />
                                ...
                                <div class="flex flex-col gap-2">
                                    <label for="nomeOrganizacao" class="text-surface-900 dark:text-surface-0 font-medium">
                                        Nome da Organização <span class="text-muted-color text-sm">(opcional)</span>
                                    </label>
                                    <input pInputText id="nomeOrganizacao" type="text" placeholder="Ex: Minha Empresa" formControlName="nomeOrganizacao" />
                                </div>

                                <p-button type="submit" label="Criar Conta" [loading]="loading()" [disabled]="registerForm.invalid || loading()" />
                            </form>
                        </p-fluid>
